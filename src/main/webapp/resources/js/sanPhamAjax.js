$(document).ready(function () {
    ...
    function ajaxGet(page) {
        ...
        $.ajax({
            ...
            url: "http://localhost:8080/techshop/api/san-pham/all" + '?page=' + page,
            success: function (result) {
                $.each(result.content, function (i, sanPham) {
                    var sanPhamRow = '<tr>' +
                        '<td>' + '<img src="/techshop/img/' + sanPham.id + '.png" class="img-responsive" style="height: 50px; width: 50px" />' + '</td>' +
                        '<td>' + sanPham.tenSanPham + '</td>' +
                        '<td>' + sanPham.danhMuc.tenDanhMuc + '</td>' +
                        '<td>' + sanPham.hangSanXuat.tenHangSanXuat + '</td>' +
                        '<td>' + sanPham.donGia.toLocaleString("it-IT") + '</td>' +
                    ...
                    var checkTenDanhMucPC = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("PC".toLowerCase());
                    sanPhamRow += (checkTenDanhMuc != -1) ? '<button class="btn btn-primary btnCapNhatLapTop" >Cập nhật</button>' : ((checkTenDanhMucPC != -1) ? '<button class="btn btn-primary btnCapNhatPC" >Cập nhật</button>' : '<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>');
    ...
    $('#danhMucDropdown').mouseup(function () {
        ...
        if (open) {
            ...
            else if((label.toLowerCase()).indexOf("PC".toLowerCase()) != -1){
                $('.PCModal').modal('show');
                $("#idDanhMucPC").val($(this).val());
                $('#PCForm').removeClass().addClass("addPCForm");
                $('#PCForm #btnSubmitPC').removeClass().addClass("btn btn-primary btnSavePCForm");
            }
            else {
    ...
    $('.lapTopModal, .otherModal,.PCModal').on('hidden.bs.modal', function (e) {
        e.preventDefault();
        $("#idDanhMucLaptop, #idDanhMucKhac").val("");
        $("#idSanPhamLapTop, #idSanPhamKhac").val("");
        $("#idSanPhamPC, #idSanPhamKhac").val("");
        ...
        $('#PCForm').removeClass().addClass("PCForm");
        $('#PCForm #btnSubmitPC').removeClass().addClass("btn btn-primary");
    ...
    function ajaxPostLapTop() {
        ...
        $.ajax({
            ...
            type: "POST",
            contentType: "application/json",
            url: "http://localhost:8080/techshop/api/san-pham/save",
    ...
    $(document).on('click', '.btnSavePCForm', function (event) {
        event.preventDefault();
        ajaxPostPC();
        resetData();
    });
    ...
    $(document).on("click", ".btnCapNhatLapTop", function (event) {
        ...
        var href = "http://localhost:8080/techshop/api/san-pham/" + sanPhamId;
    ...
    // click cap nhat button vs danh muc PC
    $(document).on("click", ".btnCapNhatPC", function (event) {
        event.preventDefault();
        var sanPhamId = $(this).parent().prev().children().val();
        $('#PCForm').removeClass().addClass("updatePCForm");
        $('#PCForm #btnSubmitPC').removeClass().addClass("btn btn-primary btnUpdatePCForm");

        var href = "http://localhost:8080/techshop/api/san-pham/" + sanPhamId;
        $.get(href, function (sanPham) {
            populate('.updatePCForm', sanPham);
            $("#idDanhMucPC").val(sanPham.danhMuc.id);
            var hangSXId = sanPham.hangSanXuat.id;
            $("#nhaSXPCId").val(hangSXId);
        });

        removeElementsByClass("error");
        $('.updatePCForm .PCModal').modal();
    });

    //btn update PC form event
    $(document).on('click', '.btnUpdatePCForm', function (event) {
        event.preventDefault();
        ajaxPutPC();
        resetData();
    });
    ...
    $(document).on("click", ".btnXoaSanPham", function () {
        ...
        if (confirmation) {
            $.ajax({
                async: false,
                type: "DELETE",
                url: "http://localhost:8080/techshop/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click", ".btnChiTiet", function () {
        ...
        var href = "http://localhost:8080/techshop/api/san-pham/" + sanPhamId;
        $.get(href, function (sanPham) {
            $('.hinhAnh').attr("src", "/techshop/img/" + sanPham.id + ".png");
            ...
            var checkTenDanhMucPC = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("PC".toLowerCase());
            console.log(checkTenDanhMucPC != -1);
            if(checkTenDanhMucPC != -1){
            ...
            $('.donGia').html("<span style='font-weight: bold'>Đơn giá: </span>" + sanPham.donGia.toLocaleString("it-IT") + " VNĐ");
    ...
    $(document).on('keyup', '#searchById', function (event) {
        ...
        if (tenSanPham != '') {
            ...
            $.ajax({
                ...
                data: tenSanPham,
                contentType: "application/json",
                url: "http://localhost:8080/techshop/api/san-pham/" + '?tenSanPham=' + tenSanPham,
                success: function (result) {
                    $('.sanPhamTable tbody tr').remove();
                    $('.pagination li').remove();
                    $.each(result.content, function (i, sanPham) {
