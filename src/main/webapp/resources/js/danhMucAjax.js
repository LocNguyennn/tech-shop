$(document).ready(function(){ // .ready: Xảy ra khi browser đã load xong
	
	// click event button Them moi danh muc
	$('.btnThemDanhMuc').on("click", function(event) {  // click : Xảy ra khi click vào đối tượng
		event.preventDefault();
		$('.danhMucForm #danhMucModal').modal(); // modal(): là cửa sổ pop-up
		$('.danhMucForm #id').prop("disabled", true); // prop: gán giá trị true cho thuộc tính disabled
		$('#form').removeClass().addClass("addForm"); // thêm giá trị cho thuộc tính class "addForm" bên quanLyDanhMuc.jsp
	...
	function ajaxGet(page){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/techshop/api/danh-muc/all" + "?page=" + page,
...
//    });
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8080/techshop/api/danh-muc/save",
    ...
    $(document).on("click",".btnCapNhatDanhMuc",function(){
		...
		var href = "http://localhost:8080/techshop/api/danh-muc/"+danhMucId;
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8080/techshop/api/danh-muc/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaDanhMuc", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/techshop/api/danh-muc/delete/" + danhMucId,
