$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/techshop/api/shipper/don-hang/all" + '?page=' + page,
...
//		event.preventDefault();
	...
	$(document).on('keyup', '#searchById', function (event){
		...
		if(donHangId != ''){
    	  $('.donHangTable tbody tr').remove();
    	  $('.pagination li').remove();
		  var href = "http://localhost:8080/techshop/api/shipper/don-hang/"+donHangId;
...
//		console.log(donHangId);
		...
		$.get(href, function(donHang) {
			...
			$("#tongTienCapNhat").text("Tổng : "+ sum.toLocaleString("it-IT"));
		});
		$("#chiTietModal").modal();
	});
...
//    	 console.log(data);
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/techshop/api/shipper/don-hang/update",
