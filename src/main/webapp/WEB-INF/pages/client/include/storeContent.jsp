			...
			<form>
				...
				<div class="select-range">
					<select name="range" class="form-control">
						...
						<option value="duoi-12-trieu">Dưới 12 triệu</option>
						<option value="12-trieu-den-17-trieu">12 triệu đến 17 triệu</option>
						<option value="17-trieu-den-25-trieu">17 triệu - 25 triệu</option>
						<option value="25-trieu-den-30-trieu">25 triệu - 30 triệu</option>
						<option value="30-trieu-den-40-trieu">30 triệu - 40 triệu</option>
						<option value="tren-40-trieu">Trên 40 triệu</option>
					</select>
				</div>
				<p>Hãng sản xuất</p>
				<div class="select-range">
		    ...
		    <div class="content-grids">
		    		...
		    		<c:if test = "${loop.index != list.size()-1}">
         				<c:if test = "${loop.index%4 == 0}">
         					...
         					<div class="grid_1_of_4 images_1_of_4 products-info"><a href="sp?id=${sanpham.id}">
									<img style="width: 300px; height: 238px" src="/techshop/img/${sanpham.id}.png">
		    	...
		    	<c:if test = "${list.size() != 0}">
            		...
            		<c:if test = "${currentPage != 1}">
                		<a href="/techshop/store?page=${currentPage-1}&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&pin=${pin}">Back</a>
                	</c:if>
                	<c:if test = "${currentPage == 1}">	
        				<a class="current">1</a>
        			</c:if>
        			<c:if test = "${currentPage != 1}">	
        				<a href="/techshop/store?page=1&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&pin=${pin}">1</a>
        			</c:if>
        			...
        			<c:forEach var="pag" items="${pageList}" varStatus="loop">
        				...
        				<c:if test = "${currentPage != pag}">	
        					<a href="/techshop/store?page=${pag}&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&pin=${pin}">${pag}</a>
        				</c:if>
      				</c:forEach>

            		<c:if test = "${currentPage != totalPage}">
                		<a href="/techshop/store?page=${currentPage+1}&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&pin=${pin}">Next</a>
                	</c:if>
				</div>
		    	</c:if>
